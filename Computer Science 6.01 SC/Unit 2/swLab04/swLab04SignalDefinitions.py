...
class MyStepSignal(Signal):
    """
    The step signal has a value of zero when negative and a value of 1 in all
    other cases.
    """
    def sample(self, n):
        if n < 0:
...
class MySummedSignal(Signal):
    """
    The summed signal is a new signal that is consturcted out of adding two
    signals together.
    """
...
class MyScaledSignal(Signal):
    """
    A scaled signal is a signal that has been scaled by a constant.
    """
    ...
    def __str__(self):
        return "(%s * %s)" % (self.c, self.s)

class MyRn(Signal):
    """
    A signal which delays another signal by d.
    """
    ...
    def sample(self, n):
        return self.s.sample(n-self.d)

    def __str__(self):
        return "Delay(%s, %s)" % (self.s, self.d)

class MyR(MyRn):
    """
    Delays a signal by one.
    """
    def __init__(self, s):
        MyRn.__init__(self, s, 1)

def myPolyR(signal, polynomial):
    delays = []
    num_coeffs = len(polynomial.coeffs)
    for i in range(num_coeffs):
        delays.append(MyScaledSignal(MyRn(signal, i), polynomial.coeffs[-(i+1)]))
    return reduce(MySummedSignal, delays, ConstantSignal(0))
