...
def get_closest_cluster(clusters, point):
    return min(clusters, key=lambda x: point.distance(x.getCentroid()))
...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    test_error_vals = []
    for k in kvals:
        training_dataset, test_dataset = randomPartition(points, .8)
        clusters, maxSmallest = kmeans(training_dataset, k, cutoff, County)
        error = 0
        for cluster in clusters:
            for point in cluster.getPoints():
                error += point.distance(cluster.getCentroid()) ** 2
        error = error ** 0.5
        error_vals.append(error)
        test_error = 0
        for point in test_dataset:
            closest_cluster = get_closest_cluster(clusters, point)
            test_error += point.distance(closest_cluster.getCentroid()) ** 2
        test_error = test_error ** 0.5
        test_error_vals.append(test_error)
        # I think that this chart is deceptive, because the number of points
        # should slightly deflate the size of the test error.
    pylab.plot(kvals, error_vals)
    pylab.plot(kvals, test_error_vals)
    pylab.xlabel("K Values")
    pylab.ylabel("Error")
    pylab.figure()

    pylab.plot(kvals, pylab.array(error_vals) / pylab.array(test_error_vals))
    pylab.show()
...
def my_county_kmeans(county):
    """
    Run 3 k-means tests with k = 50 and see where your county is grouped. Spend
    some time thinking about the relationship between your county and the other
    counties in the cluster.
    """
    k = 50
    for i in range(3):
        clusters, waste = kmeans(points, k, .1, County)
        for cluster in clusters:
            if any(map(lambda x: x.name == county, cluster.getPoints())):
                print cluster


my_county_kmeans("CAOrange")
